...
public class SignupFragment extends Fragment {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        signUpViewModel.getSignUpResponse().observe(getViewLifecycleOwner(), returnCode -> {
            if(returnCode.getReturnCode() == 200) {
                signUpViewModel.createToken();
            }
        });

        signUpViewModel.getCreateTokenResponse().observe(getViewLifecycleOwner(), returnCode -> {
            if(returnCode.getReturnCode() == 200) {
